...
abstract-plan constraints until it finds a concrete plan. Each failed solve
yields an unsatisfiable core, and the planner relaxes the latest constraint the
core blames, leaving constraints that cause no conflict in place to keep guiding
the search. If the concrete task remains unsatisfiable once the core no longer
blames an active constraint, the planner continues unconstrained concrete search
above the abstract horizon.
