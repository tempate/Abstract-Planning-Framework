...
def _create_abstraction(problem, object_names, requested_abstract_name):
    objects_by_name = {item.name.casefold(): item for item in problem.all_objects}

    # Normalize names and remove duplicates.
    requested_names = _normalize_object_names(object_names)
    ...
    unknown_names = [name for name in requested_names if name not in objects_by_name]
    if unknown_names:
        raise AbstractionError(f"Unknown problem objects: {', '.join(unknown_names)}")
    objects_to_collapse = tuple(objects_by_name[name] for name in requested_names)
    ...
    abstract_name = requested_abstract_name or f"{objects_to_collapse[0].type.name}_abs"
...
def _normalize_object_names(object_names):
    ...
    for name in object_names:
        normalized_names.add(str(name).casefold())
