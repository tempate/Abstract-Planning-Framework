...
from core.abstraction.model import abstract_problem, rank_symmetry_classes


def prepare_abstraction(
    domain_path, problem_path, *, objects_to_abstract=None, abstract_name=None, symmetry_time_limit=300
):
    ...
    if selected is None:
        classes = find_symmetric_object_sets(domain_path, problem_path, symmetry_time_limit)
        if not classes:
            return None
