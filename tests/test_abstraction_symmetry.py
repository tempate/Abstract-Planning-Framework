...
"""
    ...
    def test_the_selected_class_reports_the_source_that_offered_it(self):
        # Ladders and symmetry classes compete in one pool, so the winner has to
        # carry its origin for the benchmark CSV to tell them apart.
        selected, _ = _select_abstraction(self.problem, [("ladder", ["tool-a", "tool-b"])])

        self.assertEqual(selected.source, "ladder")
